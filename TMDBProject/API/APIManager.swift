...
class TMDBAPIManager {
    ...
    /// 여기서 내가 타입을 명시하지 않게 된다면 All List VIew Con 가서 보면 타입을 명시하는 부분이 필요해지게 된다,.
    func fetchOnlyForAllListView<T: onlySearchViewFetch>(api: TMDBAPI, pageNum: Int, complitionHandler : @escaping (T) -> Void) {
//
//        let pageParaMetter: Parameters = ["page":pageNum]
//        let parametter = api.parametter + pageParaMetter
        // 딕셔너리 형태라 이렇게 해야함 ....!!!
        var parametter = api.parametter
        parametter["page"] = pageNum
    ...
    func fetchOnlyForAllListView<T: onlySearchViewFetch>(type: T.Type ,api: TMDBAPI, pageNum: Int, complitionHandler : @escaping (T) -> Void) {
        var parametter = api.parametter
        parametter["page"] = pageNum
